...
```
>透過 cifar10.load_data() 載入 CIFAR-10，取得原始訓練集與測試集。
...
```
>CIFAR-10 的影像大小為 32×32×3，原始訓練集共有 50000 張，測試集共有 10000 張。

```python
x_train_full = x_train_full.astype("float32") / 255.0
x_test = x_test.astype("float32") / 255.0
```
> 正規化：將影像像素值從原本的 0-255 轉成 0-1 範圍。

```python
y_train_full = y_train_full.reshape(-1)
y_test = y_test.reshape(-1)
```
>原始載入之標籤資料形狀為 (n,1)，為二維陣列。為了配合 sparse_categorical_crossentropy 的輸入格式，這邊使用 reshape(-1) 將標籤轉換為一維向量 (n,)。
...
```
>隨機打亂資料順序，再用同一組順序同步重排圖片和標籤，使後續切分出的訓練集與驗證集具有較均勻的資料分布。
...
```
>將已打亂的 50000 筆訓練資料切成 40000 筆 training set 和 10000 筆 validation set。
...
```
>增加訓練樣本的多樣性，使模型不易過度依賴固定位置、方向或姿態的特徵。
...
>此模型先透過資料增強模組提升訓練樣本多樣性，再利用三個卷積區塊逐步提取影像特徵。卷積區塊中結合 Batch Normalization、ReLU、Max Pooling 與 Dropout，以提升訓練穩定性並降低過擬合風險。最後透過 Global Average Pooling 與全連接層完成特徵整合，並以 softmax 輸出 CIFAR-10 十個類別的分類機率。
...
```
>當驗證表現長時間沒有進步時，自動停止訓練，避免浪費時間，也降低過擬合風險。
...
```
>寫入 outputs/final_result.txt：train/val/test accuracy、loss。
...
![accuracy_loss_curve](c:\Users\user\Desktop\program\ai\outputs\accuracy_loss_curve.png)
